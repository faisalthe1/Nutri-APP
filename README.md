...
Uses FatSecret Premier OAuth 2.0 (`basic premier` scopes) with `foods/search/v5` and `food/v5`, US data. Both search and details use the same structured nutrients and suggested serving. The initial search is McDonald’s, with restaurant shortcuts and free-text search. Only access tokens are cached, with expiry and one retry for expired authentication. Food content is fetched fresh. Favorites persist only namespaced food IDs; their display data is fetched on demand, six per page. Legacy Nutritionix favorites retain their original snapshots. Search shows each provider serving size; generic foods fall back to 100 g when no suggested serving is supplied. Missing nutrition is never presented as zero. FatSecret attribution appears in the shared footer.

Render cleanup: obsolete `NUTRITIONIX_APP_ID` and `NUTRITIONIX_APP_KEY` are no longer needed. Virtual environments, bytecode, IDE files, and the local SQLite database are excluded from Git; Render installs dependencies and creates the database through migrations on startup. For CIDR access, authorize `74.220.48.0/24` and `74.220.56.0/24` in the FatSecret account (confirm current ranges in Render’s Connect menu).
