...
2. Set `NUTRITIONIX_APP_ID` and `NUTRITIONIX_APP_KEY`. The previous code committed provider credentials: rotate those credentials with Nutritionix and store the replacement only in Render.
...
The regression suite mocks Nutritionix, so it needs no live credentials. It covers guest and account flows, input validation, saved-meal ownership, API failures, caching, and malformed responses. `/healthz/` checks that the web process responds; it does not assert third-party API or database readiness.
